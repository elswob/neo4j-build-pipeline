...
def protein_to_pathway():
    ...
    print(df.head())
    df.to_csv(os.path.join(data_dir,f'UniProt2Reactome_All_Levels_human_{today}.csv'),index=False)
...
def pathways():
    ...
    print(df1.head())
    df1.to_csv(os.path.join(data_dir,f'ReactomePathways_human_{today}.csv'),index=False)
    ...
    print(df2.shape)
    df2.to_csv(os.path.join(data_dir,f'ReactomePathwaysRelation_human_{today}.csv'),index=False)
