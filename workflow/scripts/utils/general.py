...
def neo4j_connect():
	...
	driver = GraphDatabase.driver("bolt://" + graph_host + ":" + graph_bolt_port, auth=auth_token, encrypted=False)
	return driver
...
def make_outDir(meta_id):
    ...
    try:
        ...
        logger.error('Something wrong with meta_id "{}", perhaps it does not exists in data-integration.yaml' ,meta_id)
        exit()
...
def create_df(data_dir,name,nrows=None):
    ...
    header_file=open(os.path.join(data_dir,name+'.header'),'r').read().rstrip().split(",")
    ...
    df = pd.read_csv(csv_file,names=header_cols,usecols=include_cols,dtype=str,nrows=nrows)
    return df
